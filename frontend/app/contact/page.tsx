...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) setForm({ name: "", email: "", phone: "", department: "", message: "" });
  };
    ...
    <>
      <section className="bg-[#0a4f3c] pt-32 pb-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 text-xl max-w-2xl">Our Nairobi team is ready to help. Choose the right channel for your inquiry.</p>
        </div>
      </section>
      ...
      <section className="bg-white py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <h2 className="text-2xl font-bold text-[#0a4f3c] mb-8">Get in touch</h2>
              <div className="space-y-6 mb-10">
                {[
                  { Icon: MapPin, label: "Address", value: "Olkasis Capital Ltd, Westlands, Nairobi, Kenya" },
                  { Icon: Mail, label: "General enquiries", value: "info@olkasiscapital.com" },
                  { Icon: Mail, label: "Investor relations", value: "investors@olkasiscapital.com" },
                  { Icon: Mail, label: "Partnerships", value: "partners@olkasiscapital.com" },
                  { Icon: Mail, label: "Press & media", value: "press@olkasiscapital.com" },
                  { Icon: Phone, label: "Phone", value: "+254 (0) 700 000 000" },
                  { Icon: Clock, label: "Support hours", value: "Monday – Friday, 8:00am – 6:00pm EAT" },
                  ...
                  <div key={label} className="flex gap-4">
                    ...
                    <div>
                      ...
                      <div className="font-medium text-[#063328] text-sm mt-0.5">{value}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <h2 className="text-2xl font-bold text-[#0a4f3c] mb-8">Send a message</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" size="lg" className="w-full gap-2" disabled={state.status === "loading"}>
                  {state.status === "loading" ? "Sending..." : <><span>Send Message</span><ArrowRight className="w-4 h-4" /></>}
                </Button>
              </form>
            </div>
          </div>
