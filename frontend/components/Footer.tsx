      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 mb-12">
          ...
          {Object.entries(footerLinks).map(([section, links]) => (
            <div key={section}>
              <h4 className="font-semibold text-sm text-white/80 mb-3 uppercase tracking-wider">{section}</h4>
        ...
        <div className="border-t border-white/10 pt-8">
          <div className="bg-white/5 rounded-xl p-4 mb-6 text-xs text-white/50 leading-relaxed">
            <strong className="text-white/70">Risk Disclosure:</strong> Investing involves risk, including the possible loss of capital. Olkasis Capital is regulated by the Capital Markets Authority (CMA) Kenya. Past performance is not indicative of future results. All investments carry risk and may not be suitable for all investors.
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-white/40">
            <p>© {new Date().getFullYear()} Olkasis Capital Ltd. All rights reserved. CMA Kenya Licensed.</p>
            <div className="flex items-center gap-4">
              <span className="bg-white/10 px-3 py-1 rounded-full">🇰🇪 CMA Kenya</span>
              <span className="bg-white/10 px-3 py-1 rounded-full">NSE Member</span>
            </div>
          </div>
        </div>
      </div>
